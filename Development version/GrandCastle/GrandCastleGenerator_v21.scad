...
module Island(r,h,over,moat,sides=4, rseed){
    ...
    union(){
        ...
        for(loop=[0:loops]){
            ...
            v1 = [ for(i=[0:l])
                    let(
                    r=rands(0,360,1,rseed+i+loop)[0],
                    t=rands(90,180,1,rseed+2*i+2*loop)[0])
                    [sin(r)*cos(t),cos(r)*cos(t),sin(t)]];
            
            v2 = [for(j=[0:n])[
                floor(rands(0,l-1,1,rseed+3*j+3*loop)[0]),
                floor(rands(0,l-1,1,rseed+4*j+4*loop)[0]),
                floor(rands(0,l-1,1,rseed+5*j+5*loop)[0])]]; 
        ...
        union(){
            ...
            hull(){
                ...
                translate([0,0,-h*2])
                rotate([0,0,rands(0,360,1,rseed)[0]])
                linear_extrude(1, 0, convexity = 3) 
                offset(r = 4)
                circle(r,$fn=sides /2);
